document.addEventListener("DOMContentLoaded", function () {
  ...
  function randomCicle() {
    ...
    for (let i = 0; i < circle.length; i++) {
      ...
      if (i < 3) {
        let randomLeftL = getRandomNumber(
          0,
          wrapWidth * 0.5 - dropZone.offsetWidth
        );
        circle[i].style.left = randomLeftL + "px";
        circle[i].style.top = randomTop + "px";
        // console.log(randomLeftL);
      } else {
        let randomLeftR = getRandomNumber(
          wrapWidth * 0.5 + dropZone.offsetWidth * 0.5,
          wrapWidth - 150
        );
  ...
  function dropEvent(num) {
    ...
    if (saveZindex === 20) {
      //saveZindex가 20일경우 처음 올려둔 이미지만 블렌드 모드를 기본으로
